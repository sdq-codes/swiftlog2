...
class PasswordResetNotifications extends Notification implements ShouldQueue
{
    ...
    public function __construct(User $user)
    {
        ...
        $this->token = $token;
        $this->respondLink = 'https://priceless-goldstine-f6e90a.netlify.app/reset/' . $this->token;
    ...
    protected function resetPassword($notifiable)
    {
        ...
        $inviteFooter = '
        Should you have  any concerns, please send us an email at support@swiftdeliveries.com';
        $viewData = array('app'=>$this->app, 'logo' => $this->logo);
        $fullname = $this->user->name;
        return (new MailMessage)->from(config('mail.from.address'), 'Swift Deliveries Support')
